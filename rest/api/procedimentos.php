<?php
	...
	$mQuery = file_get_contents("../sql/procedimentos_WhatsOUT.sql");
	...
	$codigo =  @$mRetorno['id_procedimento'];
	...
	if($codigo){
		...
		$mQuery = str_replace("__procedimento",$codigo, $mQuery);
		...
		if($dataDB ){
			...
			foreach($dataDB as  $arrRow )
			{
				...
				$mContent = json_decode($arrRow["procedimentos"]);
